...
tf.compat.v1.flags.DEFINE_string('w2v_file', './data/ECF_glove_300.txt', 'embedding file')
tf.compat.v1.flags.DEFINE_string('path', './data/', 'path for dataset')
tf.compat.v1.flags.DEFINE_string('video_emb_file', './data/video_embedding_4096.npy', 'ndarray (13620, 4096)')
tf.compat.v1.flags.DEFINE_string('audio_emb_file', './data/audio_embedding_6373.npy', 'ndarray (13620, 6373)')
tf.compat.v1.flags.DEFINE_string('video_idx_file', './data/video_id_mapping.npy', 'mapping dict: {dia1utt1: 1, ...}')
tf.compat.v1.flags.DEFINE_integer('embedding_dim', 300, 'dimension of word embedding')
tf.compat.v1.flags.DEFINE_integer('embedding_dim_pos', 50, 'dimension of position embedding')
## input struct ##
tf.compat.v1.flags.DEFINE_integer('max_sen_len', 35, 'max number of tokens per sentence') 
tf.compat.v1.flags.DEFINE_integer('max_doc_len', 35, 'max number of sentences per document') 
tf.compat.v1.flags.DEFINE_integer('max_sen_len_bert', 40, 'max number of tokens per sentence') 
tf.compat.v1.flags.DEFINE_integer('max_doc_len_bert', 400, 'max number of tokens per document for Bert Model') 
## model struct ##
tf.compat.v1.flags.DEFINE_string('model_type', 'BiLSTM', 'model type: BERTcased, BERTuncased, BiLSTM')
tf.compat.v1.flags.DEFINE_string('bert_encoder_type', 'BERT_sen', 'model encoder type: BERT_doc, BERT_sen')
tf.compat.v1.flags.DEFINE_string('bert_base_dir', './data/cased_L-12_H-768_A-12/', 'base dir of pretrained bert') 
tf.compat.v1.flags.DEFINE_string('share_word_encoder', 'yes', 'whether emotion and cause share the same underlying word encoder')
tf.compat.v1.flags.DEFINE_string('choose_emocate', '', 'whether predict the emotion category')
tf.compat.v1.flags.DEFINE_string('use_x_v', '', 'whether use video embedding')
tf.compat.v1.flags.DEFINE_string('use_x_a', '', 'whether use audio embedding')
...
tf.compat.v1.flags.DEFINE_float('bert_hidden_kb', 0.9, 'keep prob for bert')
tf.compat.v1.flags.DEFINE_float('bert_attention_kb', 0.7, 'keep prob for bert')
tf.compat.v1.flags.DEFINE_integer('end_run', 21, 'end_run')
tf.compat.v1.flags.DEFINE_float('keep_prob1', 1.0, 'keep prob for word embedding')
tf.compat.v1.flags.DEFINE_float('keep_prob2', 1.0, 'keep prob for softmax layer')
tf.compat.v1.flags.DEFINE_float('keep_prob_v', 0.5, 'training dropout keep prob for visual features')
tf.compat.v1.flags.DEFINE_float('keep_prob_a', 0.5, 'training dropout keep prob for audio features')
tf.compat.v1.flags.DEFINE_float('l2_reg', 1e-5, 'l2 regularization')
tf.compat.v1.flags.DEFINE_float('emo', 1., 'loss weight of emotion ext.')
tf.compat.v1.flags.DEFINE_float('cause', 1., 'loss weight of cause ext.')
tf.compat.v1.flags.DEFINE_integer('training_iter', 15, 'number of training iter')

tf.compat.v1.flags.DEFINE_string('log_path', './log', '')
tf.compat.v1.flags.DEFINE_string('scope', 'TEMP', 'scope')
tf.compat.v1.flags.DEFINE_string('log_file_name', 'step1.log', 'name of log file')
...
def build_subtasks(embeddings, placeholders):
    ...
    x = tf.nn.dropout(x, rate = 1 - (is_training * FLAGS.keep_prob1 + (1.-is_training)))
    
    x_a = tf.nn.embedding_lookup(audio_embedding, x_v)
    x_v = tf.nn.embedding_lookup(video_embedding, x_v)
    x_a = tf.nn.dropout(x_a, rate = 1 - (is_training * FLAGS.keep_prob_a + (1.-is_training)))
    x_v = tf.nn.dropout(x_v, rate = 1 - (is_training * FLAGS.keep_prob_v + (1.-is_training)))
    ...
    def get_s(inputs, sen_len, name):
        with tf.compat.v1.name_scope('word_encode'): 
        ...
        with tf.compat.v1.name_scope('word_attention'):
    ...
    x_v_c = x_v
    x_v = tf.nn.relu(layer_normalize(tf.compat.v1.layers.dense(x_v, h2, use_bias=True)))
    x_v_c = tf.nn.relu(layer_normalize(tf.compat.v1.layers.dense(x_v_c, h2, use_bias=True)))
    pred_emo_x_v, w_emo_x_v, b_emo_x_v = emo_cause_prediction(x_v, is_training, name='emotion_x_v')

    x_a_c = x_a
    x_a = tf.nn.relu(layer_normalize(tf.compat.v1.layers.dense(x_a, h2, use_bias=True)))
    x_a_c = tf.nn.relu(layer_normalize(tf.compat.v1.layers.dense(x_a_c, h2, use_bias=True)))
    ...
    if FLAGS.model_type=='BiLSTM':
        ...
        s_emo = tf.compat.v1.layers.dense(s_emo, h2, use_bias=True) 
        
        s_cause = concate_feature(s_bert_cause, x_v_c, x_a_c) 
        s_cause = s_cause * feature_mask
        s_cause = tf.compat.v1.layers.dense(s_cause, h2, use_bias=True)
    ...
    while True:
        ...
        def init_from_bert_checkpoint():
            ...
            if not FLAGS.share_word_encoder:
                ...
                tf.compat.v1.train.init_from_checkpoint(init_checkpoint, assignment_map_cause)
