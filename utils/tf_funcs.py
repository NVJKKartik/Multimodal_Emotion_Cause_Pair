...
class Saver(object):
    def __init__(self, sess, save_dir, max_to_keep=10):
        ...
        self.saver = tf.compat.v1.train.Saver(write_version=tf.compat.v1.train.SaverDef.V2, max_to_keep=max_to_keep)
...
def multihead_attention(queries,
    ...
    with tf.compat.v1.variable_scope(scope):
        # Linear projections
        Q = tf.compat.v1.layers.dense(queries, units_query, activation=tf.nn.relu)  # (N, T_q, C)
        K = tf.compat.v1.layers.dense(keys, units_query, activation=tf.nn.relu)  # (N, T_k, C)
        V = tf.compat.v1.layers.dense(values, units_query, activation=tf.nn.relu)  # (N, T_k, C)
...
def pw_feedforward(inputs, num_units, out_units):
    u1 = tf.compat.v1.layers.dense(inputs, num_units, use_bias=True, activation=tf.nn.relu)  # (N, T_q, C)
    outputs = tf.compat.v1.layers.dense(u1, out_units, use_bias=True)
    return outputs
